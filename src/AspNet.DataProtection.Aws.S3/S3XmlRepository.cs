    ...
    public sealed class S3XmlRepository : IXmlRepository
    {
        ...
        private async Task<XElement> GetElementFromKey(S3Object item, SemaphoreSlim throttler, CancellationToken ct)
        {
            ...
            try
            {
                ...
                var gr = new GetObjectRequest { BucketName = Config.Bucket, Key = item.Key, ServerSideEncryptionCustomerMethod = ServerSideEncryptionCustomerMethod.None };
                ...
                using(var response = await s3Client.GetObjectAsync(gr, ct).ConfigureAwait(false))
                {
                    ...
                    using(var md5 = testChecksum ? MD5.Create() : null)
                    {
                        ...
                        if(testChecksum)
                        {
                            ...
                            {
                                throw new
                                    InvalidOperationException($"Streamed S3 data has MD5 of {md5Value} which does not match provided MD5 metadata {headerChecksum} - corruption in transit");
                            }
                        }
        ...
        {
            var gomr = new GetObjectMetadataRequest { BucketName = Config.Bucket, Key = key, ServerSideEncryptionCustomerMethod = ServerSideEncryptionCustomerMethod.None };
